# Name: (Classification) UDV training framework part - model 3
...
def udv_frame_m_uv(model, optimizer, loss_fn, train_loader, val_loader, num_epochs, device, constraints_uv):
    ...
    for epoch_index in range (1, num_epochs + 1):
        model, train_loss_buffer, train_acc_buffer = class_trainLoop_m_uv(model,
                                                                          optimizer,
                                                                          loss_fn,
                                                                          train_loader,
                                                                          device,
...
def class_trainLoop_m_uv(model, optimizer, loss_fn, train_loader, device, constraints_uv):
    model.train()
    train_batch_loss = 0
    train_batch_acc = 0
