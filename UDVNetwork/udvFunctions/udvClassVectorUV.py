# Name: (Classification) UDV training framework part - model 1
...
def udv_frame_v_uv(model, optimizer, loss_fn, train_loader, val_loader, num_epochs, device, constraints_u, constraints_v):
    ...
    for epoch_index in range (1, num_epochs + 1):
        model, train_loss_buffer, train_acc_buffer = class_trainLoop_v_uv(model,
                                                                          optimizer,
                                                                          loss_fn,
                                                                          train_loader,
                                                                          device,
...
def class_trainLoop_v_uv(model, optimizer, loss_fn, train_loader, device, constraints_u, constraints_v):
    model.train()
    train_batch_loss = 0
    train_batch_acc = 0
